...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    // ════════════════════════════════════════════════════════════════════════
    // TODAY'S WORKOUT SCHEDULING
    // ════════════════════════════════════════════════════════════════════════

    /** Returns the WorkoutDay scheduled for today based on the user's active split schedule. */
    fun getTodayScheduledWorkoutDay(username: String): WorkoutDay? {
        val activeSplit = getActiveSplit(username) ?: return null
        val cal = java.util.Calendar.getInstance()
        val javaDow = cal.get(java.util.Calendar.DAY_OF_WEEK)
        val weekday = if (javaDow == java.util.Calendar.SUNDAY) 7 else javaDow - 1
        return getWeeklyScheduleForSplit(username, activeSplit.id)[weekday]
    }

    /** Weekly schedule: maps weekday index (1=Mon..7=Sun) → WorkoutDay or null for rest */
    fun getWeeklyScheduleForSplit(username: String, splitId: Int): Map<Int, WorkoutDay?> {
        val result = mutableMapOf<Int, WorkoutDay?>()
        val db = readableDatabase
        val cursor = db.rawQuery(
            "SELECT weekday, workoutDayId, isRest FROM $TABLE_WEEKLY_SCHEDULE WHERE username=? AND splitId=?",
            arrayOf(username, splitId.toString())
        )
        while (cursor.moveToNext()) {
            val weekday = cursor.getInt(0)
            val isRest = cursor.getInt(2) == 1
            if (isRest) {
                result[weekday] = null
            } else {
                val dayId = cursor.getInt(1)
                result[weekday] = getWorkoutDayById(dayId)
            }
        }
        cursor.close()
        return result
    ...
    fun saveWeeklyScheduleEntry(username: String, splitId: Int, weekday: Int, workoutDayId: Int, isRest: Boolean) {
        val db = writableDatabase
        db.delete(TABLE_WEEKLY_SCHEDULE, "username=? AND splitId=? AND weekday=?", arrayOf(username, splitId.toString(), weekday.toString()))
        val cv = ContentValues().apply {
            put("username", username)
            put("splitId", splitId)
            put("weekday", weekday)
            put("workoutDayId", workoutDayId)
            put("isRest", if (isRest) 1 else 0)
        }
        db.insert(TABLE_WEEKLY_SCHEDULE, null, cv)
    }

    fun getActiveSplit(username: String): WorkoutSplit? {
        val db = readableDatabase
        val cursor = db.rawQuery(
            "SELECT * FROM $TABLE_SPLITS WHERE username=? AND isActive=1 ORDER BY createdAt DESC LIMIT 1",
            arrayOf(username)
        )
        return if (cursor.moveToFirst()) cursorToSplit(cursor).also { cursor.close() } else { cursor.close(); null }
    }

    fun setActiveSplit(username: String, splitId: Int) {
        val db = writableDatabase
        db.execSQL("UPDATE $TABLE_SPLITS SET isActive=0 WHERE username=?", arrayOf(username))
        db.execSQL("UPDATE $TABLE_SPLITS SET isActive=1 WHERE id=? AND username=?", arrayOf(splitId, username))
    }
    ...
    fun updateSplit(splitId: Int, username: String, name: String, description: String, goal: String) {
        val db = writableDatabase
        val cv = ContentValues().apply { put("name", name); put("description", description); put("goal", goal) }
        db.update(TABLE_SPLITS, cv, "id=? AND username=?", arrayOf(splitId.toString(), username))
    }

    fun deleteSplit(splitId: Int, username: String) {
        val db = writableDatabase
        db.delete(TABLE_SPLITS, "id=? AND username=?", arrayOf(splitId.toString(), username))
        db.delete(TABLE_WORKOUT_DAYS, "splitId=?", arrayOf(splitId.toString()))
        db.delete(TABLE_WEEKLY_SCHEDULE, "splitId=? AND username=?", arrayOf(splitId.toString(), username))
    }

    fun getWorkoutDayById(dayId: Int): WorkoutDay? {
        val db = readableDatabase
        val cursor = db.rawQuery("SELECT * FROM $TABLE_WORKOUT_DAYS WHERE id=?", arrayOf(dayId.toString()))
        return if (cursor.moveToFirst()) cursorToWorkoutDay(cursor).also { cursor.close() } else { cursor.close(); null }
    }
    ...
    fun getWorkoutDaysForSplit(splitId: Int): List<WorkoutDay> {
        ...
        val cursor = db.rawQuery("SELECT * FROM $TABLE_WORKOUT_DAYS WHERE splitId=? ORDER BY sortOrder ASC", arrayOf(splitId.toString()))
        val list = mutableListOf<WorkoutDay>()
        while (cursor.moveToNext()) list.add(cursorToWorkoutDay(cursor))
        cursor.close()
        return list
    }
    ...
    fun getDayExercisesForDay(dayId: Int): List<DayExercise> {
        val db = readableDatabase
        val cursor = db.rawQuery("SELECT * FROM $TABLE_DAY_EXERCISES WHERE workoutDayId=? ORDER BY sortOrder ASC", arrayOf(dayId.toString()))
        val list = mutableListOf<DayExercise>()
        while (cursor.moveToNext()) list.add(cursorToDayExercise(cursor))
        cursor.close()
        return list
    }

    fun insertDayExercise(dayId: Int, name: String, muscleGroup: String, equipment: String, sets: Int, reps: Int, order: Int): Long {
        val db = writableDatabase
        val cv = ContentValues().apply {
            put("workoutDayId", dayId)
            put("exerciseName", name)
            put("muscleGroup", muscleGroup)
            put("equipment", equipment)
            put("defaultSets", sets)
            put("defaultReps", reps)
            put("sortOrder", order)
        }
        return db.insert(TABLE_DAY_EXERCISES, null, cv)
    }
    ...
    fun getSessionById(sessionId: Int): WorkoutSession? {
        val db = readableDatabase
        val cursor = db.rawQuery("SELECT * FROM $TABLE_SESSIONS WHERE id=?", arrayOf(sessionId.toString()))
        return if (cursor.moveToFirst()) cursorToSession(cursor).also { cursor.close() } else { cursor.close(); null }
    }

    fun updateSession(session: WorkoutSession) {
        val db = writableDatabase
        val cv = ContentValues().apply {
            put("mood", session.mood)
            put("energyLevel", session.energyLevel)
            put("sleepQuality", session.sleepQuality)
            put("notes", session.notes)
        }
        db.update(TABLE_SESSIONS, cv, "id=?", arrayOf(session.id.toString()))
    }

    fun getTotalSessionCount(username: String): Int {
        val db = readableDatabase
        val cursor = db.rawQuery("SELECT COUNT(*) FROM $TABLE_SESSIONS WHERE username=?", arrayOf(username))
    ...
    fun getLogsForSession(sessionId: Int): List<SessionExerciseLog> {
        val db = readableDatabase
        val cursor = db.rawQuery("SELECT * FROM $TABLE_SESSION_LOGS WHERE sessionId=?", arrayOf(sessionId.toString()))
        val list = mutableListOf<SessionExerciseLog>()
        while (cursor.moveToNext()) list.add(cursorToSessionLog(cursor))
        cursor.close()
        return list
    }
    ...
    fun getExerciseSessionHistory(username: String, exerciseName: String): List<SessionExerciseLog> {
        ...
        val cursor = db.rawQuery("""
            SELECT l.*, s.date FROM $TABLE_SESSION_LOGS l
            JOIN $TABLE_SESSIONS s ON l.sessionId = s.id
            WHERE s.username=? AND l.exerciseName=?
            ORDER BY s.date DESC
        """.trimIndent(), arrayOf(username, exerciseName))
        val list = mutableListOf<SessionExerciseLog>()
    ...
    fun insertSessionExerciseLog(log: SessionExerciseLog): Long {
        ...
        val cv = ContentValues().apply {
            ...
            put("muscleGroup", log.muscleGroup)
            put("weight", log.weight)
            put("sets", log.sets)
            put("reps", log.reps)
            put("isNewPR", if (log.isNewPR) 1 else 0)
            put("notes", log.notes)
            put("sortOrder", log.sortOrder)
        }
        return db.insert(TABLE_SESSION_LOGS, null, cv)
    }
    ...
    fun getPlannedWorkoutForDate(username: String, date: String): PlannedWorkout? {
        val db = readableDatabase
        val cursor = db.rawQuery("SELECT * FROM $TABLE_PLANNED WHERE username=? AND date=? LIMIT 1", arrayOf(username, date))
        return if (cursor.moveToFirst()) cursorToPlannedWorkout(cursor).also { cursor.close() } else { cursor.close(); null }
    }

    fun updatePlannedWorkoutStatus(username: String, date: String, status: String, sessionId: Int) {
        val db = writableDatabase
        val cv = ContentValues().apply { put("status", status); put("sessionId", sessionId) }
        db.update(TABLE_PLANNED, cv, "username=? AND date=?", arrayOf(username, date))
    }
    ...
    data class ExerciseStats(
        ...
        val daysSinceLastSession: Int = -1,
        val weightHistory: List<Pair<String, Double>> = emptyList(),
        val volumeHistory: List<Pair<String, Double>> = emptyList()
    )

    fun getExerciseStats(username: String, exerciseName: String): ExerciseStats {
        val sessionHistory = getExerciseSessionHistory(username, exerciseName)
        val legacyHistory = getWeightProgressForExercise(username, exerciseName)

        if (sessionHistory.isEmpty() && legacyHistory.isEmpty()) return ExerciseStats()

        val allWeights = sessionHistory.map { it.weight } + legacyHistory.map { it.second }
        val bestWeight = if (allWeights.isNotEmpty()) allWeights.max() else 0.0
        val avgWeight = if (allWeights.isNotEmpty()) allWeights.average() else 0.0
        val currentWeight = sessionHistory.firstOrNull()?.weight ?: legacyHistory.lastOrNull()?.second ?: 0.0
        val totalSessions = sessionHistory.map { it.sessionId }.distinct().size + legacyHistory.size
        val totalSets = sessionHistory.sumOf { it.sets }
        val maxReps = sessionHistory.maxOfOrNull { it.reps } ?: 0
        val totalVolume = sessionHistory.sumOf { it.weight * it.sets * it.reps }
        val estimated1RM = if (currentWeight > 0 && sessionHistory.isNotEmpty()) {
            val recent = sessionHistory.first()
            currentWeight * (1 + recent.reps / 30.0)
        } else 0.0

        val daysSince = if (sessionHistory.isNotEmpty()) {
            val sdf = java.text.SimpleDateFormat("dd/MM/yyyy", java.util.Locale.getDefault())
            try {
                val lastDate = sdf.parse(sessionHistory.first().date) ?: java.util.Date()
                val diffMs = java.util.Date().time - lastDate.time
                (diffMs / (1000 * 60 * 60 * 24)).toInt()
            } catch (e: Exception) { -1 }
        } else -1

        val weightHistory = sessionHistory.map { Pair(it.date, it.weight) }.takeLast(20)
        val volumeHistory = sessionHistory.groupBy { it.date }
            .map { (date, logs) -> Pair(date, logs.sumOf { it.weight * it.sets * it.reps }) }
            .takeLast(20)
    ...
    fun getPRHistoryForExercise(username: String, exerciseName: String): List<Pair<String, Double>> {
        val history = getExerciseSessionHistory(username, exerciseName)
        val prTimeline = mutableListOf<Pair<String, Double>>()
        var maxSoFar = 0.0
        for (log in history.reversed()) {
            if (log.weight > maxSoFar) {
                maxSoFar = log.weight
                prTimeline.add(Pair(log.date, log.weight))
            }
        }
        return prTimeline.reversed()
    }
    ...
    fun getExerciseComparisonStats(username: String, exerciseName: String): Map<String, Any> {
        ...
        cal.add(java.util.Calendar.DAY_OF_YEAR, -7)
        val twoWeeksStr = sdf.format(cal.time)

        val allHistory = getExerciseSessionHistory(username, exerciseName)
        val thisWeek = allHistory.filter { it.date >= lastWeekStr }
        val lastWeek = allHistory.filter { it.date >= twoWeeksStr && it.date < lastWeekStr }

        return mapOf(
            "thisWeekVolume" to thisWeek.sumOf { it.weight * it.sets * it.reps },
            "lastWeekVolume" to lastWeek.sumOf { it.weight * it.sets * it.reps },
            "thisWeekBestWeight" to (thisWeek.maxOfOrNull { it.weight } ?: 0.0),
            "lastWeekBestWeight" to (lastWeek.maxOfOrNull { it.weight } ?: 0.0)
        )
    }
    ...
    fun getAllExerciseNames(username: String): List<String> {
        ...
        val c1 = db.rawQuery("SELECT DISTINCT exerciseName FROM $TABLE_WORKOUT WHERE username=?", arrayOf(username))
        while (c1.moveToNext()) names.add(c1.getString(0) ?: "")
        c1.close()
        val c2 = db.rawQuery("SELECT DISTINCT exerciseName FROM $TABLE_SESSION_LOGS l JOIN $TABLE_SESSIONS s ON l.sessionId=s.id WHERE s.username=?", arrayOf(username))
        while (c2.moveToNext()) names.add(c2.getString(0) ?: "")
        c2.close()
    ...
    fun unlockExerciseMilestone(username: String, exerciseName: String, milestoneKey: String) {
        ...
        val existing = db.rawQuery(
            "SELECT id FROM $TABLE_MILESTONES WHERE username=? AND exerciseName=? AND milestoneKey=?",
            arrayOf(username, exerciseName, milestoneKey)
        )
        if (!existing.moveToFirst()) {
            val cv = ContentValues().apply {
                put("username", username)
                put("exerciseName", exerciseName)
                put("milestoneKey", milestoneKey)
                put("unlockedAt", sdf.format(java.util.Date()))
            }
            db.insert(TABLE_MILESTONES, null, cv)
        }
        existing.close()
    }
    ...
    fun getMilestonesForExercise(username: String, exerciseName: String): Map<String, String> {
        val db = readableDatabase
        val cursor = db.rawQuery(
            "SELECT milestoneKey, unlockedAt FROM $TABLE_MILESTONES WHERE username=? AND exerciseName=?",
            arrayOf(username, exerciseName)
        )
        val map = mutableMapOf<String, String>()
        while (cursor.moveToNext()) {
    ...
    fun getMeasurementTrend(username: String, fieldName: String): List<Pair<String, Double>> {
        val db = readableDatabase
        val safeField = when (fieldName) {
        ...
        val cursor = db.rawQuery(
            "SELECT date, $safeField FROM ${TABLE_MEASUREMENTS} WHERE username=? AND $safeField > 0 ORDER BY date ASC LIMIT 30",
            arrayOf(username)
        )
        val list = mutableListOf<Pair<String, Double>>()
        while (cursor.moveToNext()) {
            list.add(Pair(cursor.getString(0) ?: "", cursor.getDouble(1)))
        }
        cursor.close()
        return list
    }
    ...
    fun getBodyWeightChangeThisMonth(username: String): Double {
        ...
        val db = readableDatabase
        val cursor = db.rawQuery(
            "SELECT bodyWeight FROM $TABLE_MEASUREMENTS WHERE username=? AND date BETWEEN ? AND ? AND bodyWeight > 0 ORDER BY date ASC",
            arrayOf(username, start, end)
        )
        val weights = mutableListOf<Double>()
        while (cursor.moveToNext()) weights.add(cursor.getDouble(0))
        cursor.close()
        return if (weights.size >= 2) weights.last() - weights.first() else 0.0
    }
    ...
    fun insertExerciseToLibrary(exercise: ExerciseInfo): Long {
        val db = writableDatabase
        val cv = ContentValues().apply {
            put("username", exercise.username)
            ...
            put("isFavorite", if (exercise.isFavorite) 1 else 0)
        }
        return db.insert(TABLE_EXERCISE_LIB, null, cv)
    }
    ...
    fun getExerciseLibrary(username: String): List<ExerciseInfo> {
        val db = readableDatabase
        val cursor = db.rawQuery("SELECT * FROM $TABLE_EXERCISE_LIB WHERE username='' OR username=? ORDER BY name ASC", arrayOf(username))
        val list = mutableListOf<ExerciseInfo>()
        while (cursor.moveToNext()) {
    ...
    // ════════════════════════════════════════════════════════════════════════
    // ADAPTIVE INSIGHTS — ADDITIONAL QUERIES
    // ════════════════════════════════════════════════════════════════════════
    ...
    fun getWeeklyWorkoutCount(username: String): Int {
        ...
        val db = readableDatabase
        val c1 = db.rawQuery("SELECT COUNT(*) FROM $TABLE_WORKOUT WHERE username=? AND date BETWEEN ? AND ?", arrayOf(username, weekAgo, today))
        val count1 = if (c1.moveToFirst()) c1.getInt(0) else 0; c1.close()
        val c2 = db.rawQuery("SELECT COUNT(*) FROM $TABLE_SESSIONS WHERE username=? AND date BETWEEN ? AND ?", arrayOf(username, weekAgo, today))
        val count2 = if (c2.moveToFirst()) c2.getInt(0) else 0; c2.close()
        return maxOf(count1, count2)
    }

    fun getLeastTrainedMuscleGroup(username: String): String {
        val dist = getMuscleGroupDistribution(username)
        return dist.minByOrNull { it.second }?.first ?: "N/A"
    }

    fun getDaysSinceLastTrainedMuscle(username: String, muscleGroup: String): Int {
        val db = readableDatabase
        val cursor = db.rawQuery("SELECT date FROM $TABLE_WORKOUT WHERE username=? AND muscleGroup=? ORDER BY date DESC LIMIT 1", arrayOf(username, muscleGroup))
        if (!cursor.moveToFirst()) { cursor.close(); return 999 }
        val dateStr = cursor.getString(0) ?: ""; cursor.close()
        val sdf = java.text.SimpleDateFormat("dd/MM/yyyy", java.util.Locale.getDefault())
        return try {
            val last = sdf.parse(dateStr) ?: return 999
            val diff = java.util.Date().time - last.time
    ...
    fun getMostImprovedExercise(username: String): String {
        val exercises = getAllExerciseNames(username)
        var bestExercise = "N/A"
        var bestImprovement = 0.0
        for (ex in exercises) {
            val improvement = getExerciseWeightImprovementLastMonth(username, ex)
            if (improvement > bestImprovement) { bestImprovement = improvement; bestExercise = ex }
        }
        return bestExercise
    }
    ...
    fun getExerciseWeightImprovementLastMonth(username: String, exerciseName: String): Double {
        ...
        val db = readableDatabase
        val cursor = db.rawQuery(
            "SELECT weight FROM $TABLE_WORKOUT WHERE username=? AND exerciseName=? AND date BETWEEN ? AND ? ORDER BY date ASC",
            arrayOf(username, exerciseName, start, end)
        )
        val weights = mutableListOf<Double>()
        while (cursor.moveToNext()) weights.add(cursor.getDouble(0))
        cursor.close()
    ...
    fun getFavouriteMuscleGroup(username: String): String {
        val dist = getMuscleGroupDistribution(username)
        return dist.maxByOrNull { it.second }?.first ?: "N/A"
    }

    fun getPersonalRecordsCount(username: String): Int = getPersonalRecords(username).size

    // cursor helpers for new tables
    private fun cursorToSplit(cursor: Cursor): WorkoutSplit = WorkoutSplit(
        id = cursor.getInt(cursor.getColumnIndexOrThrow("id")),
        username = cursor.getString(cursor.getColumnIndexOrThrow("username")) ?: "",
        name = cursor.getString(cursor.getColumnIndexOrThrow("name")) ?: "",
        description = cursor.getString(cursor.getColumnIndexOrThrow("description")) ?: "",
        goal = cursor.getString(cursor.getColumnIndexOrThrow("goal")) ?: "",
        daysPerWeek = cursor.getInt(cursor.getColumnIndexOrThrow("daysPerWeek")),
        difficulty = cursor.getString(cursor.getColumnIndexOrThrow("difficulty")) ?: "",
        isActive = cursor.getInt(cursor.getColumnIndexOrThrow("isActive")) == 1
    )
    ...
    private fun cursorToSessionLog(cursor: Cursor): SessionExerciseLog {
        val dateIdx = cursor.getColumnIndex("date")
        return SessionExerciseLog(
            ...
            notes = cursor.getString(cursor.getColumnIndexOrThrow("notes")) ?: "",
            sortOrder = cursor.getInt(cursor.getColumnIndexOrThrow("sortOrder")),
            date = if (dateIdx >= 0) cursor.getString(dateIdx) ?: "" else ""
        )
    }
}
